...
/***************************************************************************************/
static int CRC16(char *in, int input_len)
{
    int i;
    int crc_value = 0;

    for (int len = 0; len <input_len; len++)
    {
        i = (in[len] ^ (crc_value >> 8)) & 0xff;
        crc_value = crcTable[i] ^ (crc_value << 8);
    }
...
bool ZPLconvertImage(AndroidBitmapInfo *info, void *pixels, char **out)
{
    ...
    *out = nullptr;
    ...
    if (ret != Z_OK)
    {
        LOGE("Deflate allocation failed");
        free(bGreyImg);
        free(bMonoImg);
        return false;
    }
    ...
    //Compress the data
    strm.next_in = reinterpret_cast<Bytef*>(bMonoImg);
    strm.avail_in = iMonoSize;
    strm.next_out = reinterpret_cast<Bytef*>(bGreyImg);
    strm.avail_out = iGreySize;
    ret = deflate(&strm, Z_FINISH);

    uint32_t compressed_size = iGreySize - strm.avail_out;
    deflateEnd(&strm);

    //Report Fail if compression failed
    if (ret != Z_STREAM_END)
    {
        LOGE("Deflate failed");
        free(bGreyImg);
    ...
    //Allocate B64 Buffer
    uint32_t allocSize = Base64::EncodedLength(compressed_size);
    char *outPtr = (char *) malloc(allocSize + 20);
    if (outPtr == nullptr)
    {
        LOGE("Memory allocation failed (Z64 Data)");
        free(bGreyImg);
    }
    *out = outPtr;

    //Return Base 64 with CRC16
    strcpy(outPtr,":Z64:");
    outPtr += 5;
    Base64::Encode((const char*) bGreyImg,compressed_size,outPtr,allocSize);
    sprintf(crc,":%04X",CRC16(outPtr, strlen(outPtr)));
    strcat(outPtr,crc);
